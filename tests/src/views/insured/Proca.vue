<template>
  <div class="md-example-child md-example-child-input-item-2">
    <al-all-head>
      <!-- 头部信息 -->
      <!-- <div slot="head_bottom" class="fixedd" style="z-index:1000">
        <van-row>
    ...
    <al-content>
      <div slot="content">
        <van-tabs v-model="active" style="z-index:101;" swipeable sticky>
          <van-tab v-for="index in 4" :title="'选项 ' + index" :key="index.id">
            内容 {{ index }}
            <div class="test_c">
              stylus样式引用测试PS蓝色背景--请忽略
            </div>
            <md-field>
              <md-input-item
                title="银行卡"
...
<script>
import { AllHead, Content } from "@/components"
import {Field, FieldItem, InputItem, Icon} from 'mand-mobile'
import { Tab, Tabs, Row, Col, Dialog } from 'vant';
